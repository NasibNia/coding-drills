...
function Human(name){
  ...
  this.birthday = function(number){
    this.age += number;
    console.log("happy birthday, you are now" + this.age + "now");
  };
}
...
blake.birthday(1);
Human.prototype.talk = function(){
  console.log("Hello, my name is " + blake.name);
};
...
console.log("ari's age is  "+ ari.age );
console.log("ari's school is  "+ ari.school );
...
// because we change the talk directly at blake.talk and not through prototype, so the method only applyes to the blake instance.
