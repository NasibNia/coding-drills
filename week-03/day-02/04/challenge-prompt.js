...
function fizzBuzzDataTypeChecker(dataTypeInput){
  ...
  if ( typeof(dataTypeInput) === "object"){
    ...
    if ( typeof(dataTypeInput) === "string"){
      customIsNum(parseInt(dataTypeInput));
...
function dataTypeArray(dataTypeInput){
  ...
  for (var i = 0; i < dataTypeInput.length ; i++){
    var elem = dataTypeInput[i];
    fizzBuzzDataTypeChecker(elem);
  }
...
function dataTypeObj(dataTypeInput){
  ...
  Object.keys(dataTypeInput).forEach(key => {
    let value = dataTypeInput[key];
    fizzBuzzDataTypeChecker(value);
  });
